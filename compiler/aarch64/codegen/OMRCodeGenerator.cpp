...
void OMR::ARM64::CodeGenerator::beginInstructionSelection()
{
    ...
    new (trHeapMemory())
        TR::ARM64AdminInstruction(OP::proc, startNode, NULL, _returnTypeInfoInstruction, self());
}
    ...
    TR::DebugCounterBase *counter, int32_t delta, TR::RegisterDependencyConditions *cond)
{
    ...
    cursor = generateTrg1MemInstruction(self(), OP::ldrimmw, node, counterReg,
        TR::MemoryReference::createWithDisplacement(self(), addrReg, 0), cursor);
    cursor
        = generateTrg1Src1ImmInstruction(self(), OP::addimmw, node, counterReg, counterReg, delta, cursor);
    cursor = generateMemSrc1Instruction(self(), OP::strimmw, node,

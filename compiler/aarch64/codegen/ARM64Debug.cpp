...
{
    printPrefix(log, instr);
    if (instr->getOpCodeValue() == OP::csincx) {
        // cset alias
        log->prints("cset \t");
        print(log, instr->getTargetRegister(), TR_WordReg);
...
void TR_Debug::print(OMR::Logger *log, TR::ARM64Trg1ImmInstruction *instr)
{
    ...
    } else if (op == OP::fmovimms || op == OP::fmovimmd || op == OP::vfmov4s
        || op == OP::vfmov2d) {
    ...
    } else if (op == OP::movzx || op == OP::movzw || op == OP::movnx
...
void TR_Debug::print(OMR::Logger *log, TR::ARM64Trg1Src1ImmInstruction *instr)
{
    ...
    TR_RegisterSizes regSize = (sf == 1) ? TR_DoubleWordReg : TR_WordReg;
    bool done = false;
    if (op == OP::subsimmx || op == OP::subsimmw || op == OP::addsimmx
        || op == OP::addsimmw) {
        done = true;
    ...
    } else if ((op == OP::subimmx || op == OP::subimmw || op == OP::addimmx
                   || op == OP::addimmw)) {
        done = true;
        log->printf("%s \t", getOpCodeName(&instr->getOpCode()));
        print(log, instr->getTargetRegister(), regSize);
    ...
    } else if (op == OP::andimmx || op == OP::andimmw || op == OP::andsimmx
        || op == OP::andsimmw || op == OP::orrimmx || op == OP::orrimmw
        || op == OP::eorimmx || op == OP::eorimmw) {
        uint32_t imm12 = instr->getSourceImmediate();
        auto immr = imm12 >> 6;
        auto imms = imm12 & 0x3f;
        auto n = instr->getNbit();
        if (op == OP::andimmx || op == OP::andsimmx || op == OP::orrimmx
            || op == OP::eorimmx) {
            uint64_t immediate;
        ...
        const uint32_t elementSizeShift = (op <= OP::vdupe2d)
            ? (op - OP::vdupe16b)
            : ((op <= OP::smovwh) ? (op - OP::smovwb)
                                              : ((op <= OP::smovxh) ? (op - OP::smovxb)
                                                                                : (op - OP::umovwb)));
...
void TR_Debug::print(OMR::Logger *log, TR::ARM64SynchronizationInstruction *instr)
{
    printPrefix(log, instr);
    const char *lim
        = getBarrierLimitationName(static_cast<OP::AArch64BarrierLimitation>(instr->getSourceImmediate()));

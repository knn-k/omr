    ...
    if ((displacement % alignment) != 0) {
        ...
        if (_baseRegister != NULL) {
            if (constantIsUnsignedImm12(displacement)) {
                ...
                generateTrg1Src2Instruction(cg, OP::addx, node, newBase, _baseRegister,
                    node->getRegister());
            } else {
...
void OMR::ARM64::MemoryReference::normalize(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (displacement != 0) {
        ...
        if (!constantIsImm9(displacement)) {
            ...
            if (_baseRegister != NULL) {
                if (constantIsUnsignedImm12(displacement)) {
                    generateTrg1Src1ImmInstruction(cg, OP::addimmx, node, newBase, _baseRegister,
                        displacement);
...
void OMR::ARM64::MemoryReference::consolidateRegisters(TR::Node *srcTree, TR::CodeGenerator *cg)
{
    ...
    if (_baseRegister != NULL) {
        if (self()->isIndexSignExtended()) {
            ...
            generateTrg1Src2ShiftedInstruction(cg, OP::addx, srcTree, tempTargetRegister, _baseRegister,
        ...
        if (self()->isIndexSignExtended()) {
            ...
            generateTrg1Src1ImmInstruction(cg, OP::sbfmx, srcTree, tempTargetRegister, tempTargetRegister,
                imm);
        }
    } else if (self()->isIndexSignExtended()) {
        uint32_t imm = (self()->isIndexSignExtendedWord() ? 31 : (self()->isIndexSignExtendedHalf() ? 15 : 7));
        generateTrg1Src1ImmInstruction(cg, OP::sbfmx, srcTree, tempTargetRegister, _indexRegister, imm);
    } else {
...
{
    return (((scale == 1)
                && ((mnemonic == OP::ldrhoff) || (mnemonic == OP::ldrshoffw)
                    || (mnemonic == OP::ldrshoffx) || (mnemonic == OP::strhoff)
                    || (mnemonic == OP::vldroffh) || (mnemonic == OP::vstroffh)))
        || ((scale == 2)
            && ((mnemonic == OP::ldroffw) || (mnemonic == OP::ldrswoff)
                || (mnemonic == OP::stroffw) || (mnemonic == OP::vldroffs)
                || (mnemonic == OP::vstroffs)))
        || ((scale == 3)
            && ((mnemonic == OP::ldroffx) || (mnemonic == OP::stroffx)
                || (mnemonic == OP::vldroffd) || (mnemonic == OP::vstroffd)))
        ...
        if (op.getMnemonic() != OP::addimmx) {
            ...
            if (constantIsUnsignedImm12(displacement)) {
                ...
                *wcursor = OP::getOpCodeBinaryEncoding(OP::movzw) | (lower << 5);
                ...
                wcursor++;
                *wcursor
                    = OP::getOpCodeBinaryEncoding(OP::movkw) | ((upper | TR::MOV_LSL16) << 5);
                immreg->setRegisterFieldRD(wcursor);
                wcursor++;
                    ...
                    if (constantIsImm9(displacement)) {
                        ...
                        if (isImm9UnscaledOffsetInstruction(enc)) {
                            if (isBaseModifiable() && constantIsUnsignedImm12(displacement)) {
                                ...
                                generateTrg1Src1ImmInstruction(cg, OP::addimmw,

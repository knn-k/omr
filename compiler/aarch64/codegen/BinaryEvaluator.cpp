 ...
 */
static inline TR::Register *genericBinaryEvaluator(TR::Node *node, OP::Mnemonic regOp,
    OP::Mnemonic regOpImm, bool is64Bit, TR::CodeGenerator *cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL) {
        ...
        if ((constantIsUnsignedImm12(value) || constantIsUnsignedImm12Shifted(value)) && regOp != regOpImm) {
            ...
            && (regOpImm == OP::addimmw || regOpImm == OP::addimmx
                || regOpImm == OP::subimmw || regOpImm == OP::subimmx)) {
            ...
            if ((secondChild->getReferenceCount() == 1)
                && (regOp == OP::addw || regOp == OP::addx || regOp == OP::subw
                    || regOp == OP::subx)
 ...
 */
static TR::Register *generateShiftedBinaryOperation(TR::Node *node, OP::Mnemonic op,
    OP::Mnemonic notOp, TR::CodeGenerator *cg)
{
    TR::Node *firstChild = node->getFirstChild();
    TR::Node *secondChild = node->getSecondChild();
    ...
    TR::Register *rhsReg, TR::CodeGenerator *cg)
{
    ...
    generateTrg1Src2Instruction(cg, OP::vbsl16b, node, resReg, (isMax ? lhsReg : rhsReg),
        (isMax ? rhsReg : lhsReg));
...
static TR::Register *idivHelper(TR::Node *node, bool is64bit, TR::CodeGenerator *cg)
{
    ...
    generateTrg1Src2Instruction(cg, is64bit ? OP::sdivx : OP::sdivw, node, trgReg, src1Reg,
        src2Reg);
...
static TR::Register *iremHelper(TR::Node *node, bool is64bit, bool isUnsigned, TR::CodeGenerator *cg)
{
    ...
    OP::Mnemonic op = is64bit ? (isUnsigned ? OP::udivx : OP::sdivx)
                                          : (isUnsigned ? OP::udivw : OP::sdivw);
    generateTrg1Src2Instruction(cg, op, node, tmpReg, src1Reg, src2Reg);
    generateTrg1Src3Instruction(cg, is64bit ? OP::msubx : OP::msubw, node, trgReg, tmpReg,
        src2Reg, src1Reg);
 ...
 * @return target register
 */
static inline TR::Register *logicBinaryEvaluator(TR::Node *node, OP::Mnemonic regOp,
    OP::Mnemonic regOpImm, OP::Mnemonic regNotOp, bool is64Bit, TR::CodeGenerator *cg)
{
